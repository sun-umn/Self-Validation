...
#--------------------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------------------
def DIP_train(learning_rate_model, OPTIMIZER, LOSS, input_depth, clean_image_path, corrupted_image_path, max_epoch, print_step,gpu,earlystop,cur_image,start_detection,Keep_track,window_size):
    ...
    net_input = get_noise(input_depth, INPUT, (img_size, img_size)).detach().to(device)
    ...
    # RMSprop
    if OPTIMIZER == 'SGD':
        optimizer_net = torch.optim.SGD(net.parameters(),learning_rate_model)
    elif OPTIMIZER == 'Adam':
    ...
    for epoch in range(max_epoch):
        ...
        for step, (X_clean, X_corrupted, idx) in enumerate(train_loader):
            ...
            if epoch>=start_detection and len(Collected_Output)==window_size and epoch % print_step == 0 and Keep_track==True:
                ...
                if AE_Stop == True:
                    ...
                    final_best_data = np.load(os.path.join(dir_name_best,'{}_best_data.npz'.format(cur_image)))['arr_0'][0,:] #3,512,512---0,1,2
                    ...
                    corrupted_ssim = compare_ssim(clean_data, corrupted_data, multichannel=True, data_range=corrupted_data.max() - corrupted_data.min())
                    rec_psnr = peak_signal_noise_ratio(clean_data, final_best_data)
                    rec_ssim = compare_ssim(clean_data, final_best_data, multichannel=True, data_range=final_best_data.max() - final_best_data.min())
        ...
        if epoch%print_step == 0:
            ...
            train_mean_psnr_corr, train_mean_psnr_rec, train_mean_ssim_corr, train_mean_ssim_rec=DIP_test(net, net_input, train_loader, device, figure_name)
            ...
            psnr_file_name = dir_name + 'psnr.png'
            display_psnr(train_psnr_rec, train_psnr_corr, print_step, loss_file=psnr_file_name)
...
#--------------------------------------------------------------------------------------------------
#--------------------------------------------------------------------------------------------------
def DIP_test(model,net_input, train_loader,device,figure_name):
    model.eval()
    with torch.no_grad():
